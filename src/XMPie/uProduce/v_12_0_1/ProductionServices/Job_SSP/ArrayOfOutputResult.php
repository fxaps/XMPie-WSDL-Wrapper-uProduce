<?php

namespace XMPieWsdlClient\XMPie\uProduce\v_12_0_1\ProductionServices\Job_SSP;

class ArrayOfOutputResult implements \ArrayAccess, \Iterator, \Countable
{

    /**
     * @var OutputResult[] $OutputResult
     */
    protected $OutputResult = null;

    
    public function __construct()
    {
    
    }

    /**
     * @return OutputResult[]
     */
    public function getOutputResult()
    {
      return $this->OutputResult;
    }

    /**
     * @param OutputResult[] $OutputResult
     * @return \XMPieWsdlClient\XMPie\uProduce\v_12_0_1\ProductionServices\Job_SSP\ArrayOfOutputResult
     */
    public function setOutputResult(array $OutputResult = null)
    {
      $this->OutputResult = $OutputResult;
      return $this;
    }

    /**
     * ArrayAccess implementation
     *
     * @param mixed $offset An offset to check for
     * @return boolean true on success or false on failure
     */
    public function offsetExists($offset): bool
    {
      return isset($this->OutputResult[$offset]);
    }

    /**
     * ArrayAccess implementation
     *
     * @param mixed $offset The offset to retrieve
     * @return OutputResult
     */
    public function offsetGet($offset): OutputResult
    {
      return $this->OutputResult[$offset];
    }

    /**
     * ArrayAccess implementation
     *
     * @param mixed $offset The offset to assign the value to
     * @param OutputResult $value The value to set
     * @return void
     */
    public function offsetSet($offset, $value): void
    {
      if (!isset($offset)) {
        $this->OutputResult[] = $value;
      } else {
        $this->OutputResult[$offset] = $value;
      }
    }

    /**
     * ArrayAccess implementation
     *
     * @param mixed $offset The offset to unset
     * @return void
     */
    public function offsetUnset($offset): void
    {
      unset($this->OutputResult[$offset]);
    }

    /**
     * Iterator implementation
     *
     * @return OutputResult Return the current element
     */
    public function current(): OutputResult
    {
      return current($this->OutputResult);
    }

    /**
     * Iterator implementation
     * Move forward to next element
     *
     * @return void
     */
    public function next(): void
    {
      next($this->OutputResult);
    }

    /**
     * Iterator implementation
     *
     * @return string|null Return the key of the current element or null
     */
    public function key(): string|null
    {
      return key($this->OutputResult);
    }

    /**
     * Iterator implementation
     *
     * @return boolean Return the validity of the current position
     */
    public function valid(): bool
    {
      return $this->key() !== null;
    }

    /**
     * Iterator implementation
     * Rewind the Iterator to the first element
     *
     * @return void
     */
    public function rewind(): void
    {
      reset($this->OutputResult);
    }

    /**
     * Countable implementation
     *
     * @return OutputResult Return count of elements
     */
    public function count(): int
    {
      return count($this->OutputResult);
    }

}
